    ...
    def wrapper(*args, **kwargs):
        if SESSION_ID not in session:
            raise UnauthorizedError(
                "The user has no defined session id, and therefore cannot be in a match. Redirecting them to the home page."
            )
...
def _get_room_id_or_raise_error(session_cache_handler: SessionCacheHandler):
    ...
    if not room_id:
        current_app.logger.warning(
            "The room id was not defined, resorting to session cache."
        )
        session_cache = session_cache_handler.get_cache_for_session(session[SESSION_ID])
        room_id = session_cache.get(ROOM_ID)
        if not room_id:
            current_app.logger.error(
                "Could not resolve the room, and therefore they are not in a match. Redirecting them to the home page."
            )
...
def _get_match_context_or_raise_error(room_id: str):
    ...
    if match_context is None:
        current_app.logger.error(
            "The player had a room id set but no associated match exists, resetting their session and redirecting them to the home page."
        )
...
def _get_player_info_or_raise_error(session_cache_handler: SessionCacheHandler):
    ...
    if player_info is None:
        ...
        if not player_info:
            current_app.logger.error(
                "Could not resolve the player information, and therefore they are not in a match. Redirecting them to the home page."
            )
